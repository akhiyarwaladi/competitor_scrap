...
while(True):
    for cat_name in cat_list:
        ...
        store_name = 'Tokopedia NOW!'
        el = driver.find_element(by=AppiumBy.ANDROID_UIAUTOMATOR, value="new UiSelector().text(\"{}\")".format(store_name))
        ...
        while(True):
            try:
                ...
                el = driver.find_element(by=AppiumBy.ANDROID_UIAUTOMATOR, value="new UiSelector().text(\"{}\")".format(category_name))
                ...
                # override as 'touch' pointer action
                actions.w3c_actions = ActionBuilder(driver, mouse=PointerInput(interaction.POINTER_TOUCH, "touch"))
                actions.w3c_actions.pointer_action.move_to_location((random.randint(200, 400)), 700)
                actions.w3c_actions.pointer_action.pointer_down()
                # actions.w3c_actions.pointer_action.pause(2)
                actions.w3c_actions.pointer_action.move_to_location((random.randint(200, 400)), 400)
                actions.w3c_actions.pointer_action.release()
        ...
        while(True):
            try:
                ...
                category_name = 'Semua Produk {}'.format(cat_name)
                el = driver.find_element(by=AppiumBy.ANDROID_UIAUTOMATOR, value="new UiSelector().text(\"{}\")".format(category_name))
            ...
            try:
                ...
                for idx,el in enumerate(li_el):
                    ...
                    if li_el[idx].startswith('Rp'):
                        if (
                            li_el[idx-1].startswith('Rp') 
                            or li_el[idx-1].startswith('1') 
                            or li_el[idx-1].startswith('+')
                            or li_el[idx-1].endswith('%')
                            or li_el[idx-1] == ''
                        ):
                            continue
